...
import { type Todo, TodoSchema } from "./todo.ts";
...
export const list = createQuery({
	...
	run: async (opts: { input: {}; ctx: Ctx }) => [...opts.ctx.todos.values()].sort((a, b) => b.createdAt - a.createdAt),
});
...
export const onChanged = createSubscription({
	...
	run: async function* (opts) {
		for await (const event of opts.events) {
			yield event as { kind: string; todo: Todo };
		}
	},
});

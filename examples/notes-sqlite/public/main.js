...
async function refreshUser() {
	...
	render(
		...
		currentUser
			? html`
				<span>${currentUser
					.email}</span> <button id="logout" class="ghost">Sign out</button>
			`
			: html`
				<button id="show-login" class="ghost">Sign in</button>
			`,
	);
	if (currentUser) {
		document.getElementById("logout").addEventListener("click", logout);
	} else {document.getElementById("show-login").addEventListener(
			"click",
			showAuth,
		);}
}
		...
		html`
			...
			<form id="auth-form">
				...
				<div style="display:flex; gap:0.5rem">
					...
					<button type="submit" class="ghost" data-action="signup">
						Create account
					</button>
				</div>

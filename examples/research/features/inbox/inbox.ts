...
export type InboxStore = {
	...
	find?(
		filter: { userId: string; readAt?: null | { $ne: null } },
	): Promise<InboxRow[]> | InboxRow[];
	countDocuments?(
		filter: { userId: string; readAt?: null | { $ne: null } },
...
export function inboxAdapter(
	opts: InboxAdapterOpts,
): NotificationAdapter & { _store: InboxStore } {
	return {

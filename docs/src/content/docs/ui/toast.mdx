...
<LiveExample title="Trigger toast notifications" height="280px"
	behaviors={["v-toaster"]}
	html={`
<button onclick="toast({ kind: 'info',    title: 'Heads up', body: 'New release available.' })">Info</button>
<button onclick="toast({ kind: 'success', title: 'Saved',    body: 'Project saved to drafts.' })">Success</button>
<button onclick="toast({ kind: 'warning', title: 'Slow',     body: 'Server is responding slowly.' })">Warning</button>
<button onclick="toast({ kind: 'error',   title: 'Failed',   body: 'Could not reach server.' })">Error</button>

<v-toaster position="bottom-end"></v-toaster>
`}
	css={`
...
`@vynjs/ui/toast` also exports a module-scope helper that finds the
nearest `<v-toaster>` in the document:

```ts
import { toast } from "@vynjs/ui/toast";
...
- [`@vynjs/ui/focus-trap`](/vyn/ui/focus-trap/) + native `<dialog>` — for interactive modal dialogs
- [`@vynjs/ui/live`](/vyn/ui/live/) — for one-off `aria-live` announcements without the toast UI

...
<LiveExample title="Arrow-key list navigation" height="260px"
	behaviors={["keyboard-nav"]}
	html={`
<p class="hint">Click an item, then use ↑/↓. Tab moves out.</p>
...
.hint { margin: 0 0 0.75rem; color: #64748b; font-size: 0.8rem; }
...
li { padding: 0.45rem 0.7rem; border-radius: 6px; cursor: pointer; user-select: none; }
li:focus-visible, li:focus { outline: none; background: #eef2ff; color: #4338ca; font-weight: 600; }
`}
/>

`@vynjs/ui/keyboard-nav` is a single module that scans the document for
...
This is the pattern most `@vynjs/ui` widgets compose internally. You
...
selection, pair with [`@vynjs/ui/select`](/vyn/ui/select/):
...
For floating menus, also pair with [`@vynjs/ui/popover`](/vyn/ui/popover/):
...
- [`@vynjs/ui/select`](/vyn/ui/select/) — selection state wired via `data-select`
- [`@vynjs/ui/popover`](/vyn/ui/popover/) — anchored positioning via `data-popover`

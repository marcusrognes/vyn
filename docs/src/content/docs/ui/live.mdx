...
<button id="polite">Announce polite</button>
<button id="assertive">Announce assertive</button>
...
`}
	js={`
import { live } from "@vyn/ui/live";

let n = 0;
document.getElementById("polite").addEventListener("click", () => {
	const msg = "Note saved (" + (++n) + ")";
	live(msg);
	document.getElementById("log").textContent = "polite: " + msg;
});
document.getElementById("assertive").addEventListener("click", () => {
	const msg = "Connection lost";
	live(msg, { assertive: true });
	document.getElementById("log").textContent = "assertive: " + msg;
});
`}
/>

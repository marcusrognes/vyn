...
<LiveExample title="Non-modal drawer with focus trap" height="320px"
	...
	html={`
<button>Outer button (still focusable)</button>
<button onclick="drawer.hidden = false">Open drawer</button>
...
<aside id="drawer" data-focus-trap hidden>
	...
	<div class="row">
		<button onclick="drawer.hidden = true">Cancel</button>
		<button onclick="drawer.hidden = true" class="primary">Save</button>
	</div>
</aside>
`}
	css={`
...
`@vynjs/ui/focus-trap` exists for the **non-modal** cases:
...
```ts
import { trapFocus } from "@vynjs/client";
...
- [`@vynjs/ui/dismiss`](/vyn/ui/dismiss/) — for Esc / outside / focus-out dismissal

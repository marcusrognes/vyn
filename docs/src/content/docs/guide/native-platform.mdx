...
entirely. The behaviors and widgets in `@vynjs/ui` are the polyfill
...
[`@vynjs/ui/anchor`](/vyn/ui/anchor/) leads with this CSS-first approach
...
No JS. No `<v-popover>`. No `@vynjs/ui/popover` for the common case.
...
Use [`@vynjs/ui/popover`](/vyn/ui/popover/) when you need anchored
...
<LiveExample title="Modal dialog with form method=dialog" height="260px"
	html={`
<button class="danger" onclick="confirmDlg.showModal()">Delete project…</button>
<p id="out" class="out">Nothing deleted yet.</p>

<dialog id="confirmDlg" onclose="out.textContent = this.returnValue === 'confirm' ? 'Project deleted.' : 'Cancelled.'">
...
[`@vynjs/ui/focus-trap`](/vyn/ui/focus-trap/) is for the rare non-dialog
...
Pair with [`@vynjs/ui/aria-describedby`](/vyn/ui/aria-describedby/) for
...
Pair with [`@vynjs/ui/scroll-into-view`](/vyn/ui/scroll-into-view/);
...
[`@vynjs/ui/copy`](/vyn/ui/copy/) behavior wraps it with feedback and

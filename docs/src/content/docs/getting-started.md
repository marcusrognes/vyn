...
- `_vyn.gen.ts` is regenerated by `deno task gen` (also run
  automatically by `deno task dev`). It re-exports every
  `*.actions.ts` and ties them into the runtime registry.
- `server.ts` calls `serve({ port })`. Drop a `staticContext` or
  `createContext` option in there as the app grows; see

...
We'll get the foundation in place: scaffold the project, install the few dependencies the tutorial needs, declare typed env, open a MongoDB
connection in `staticContext`, configure SuperJSON as the wire transformer, and wire Tailwind into the SPA shell.
...
The scaffolder produced a hello-world app. Strip the example feature and start from a clean slate:
...
SuperJSON is the canonical implementation of the transformer contract Vyn expects (`{ serialize, deserialize }`). Wrap it in a shared module
...
We'll import this from `server.ts` and from the client entry — both sides must use the same instance for the wire to round-trip.
...
export type ResearchRunDoc = {
	_id: string;
	userId: string;
	topic: string;
	status: "queued" | "running" | "completed" | "failed";
	result?: { summary: string; citations: string[]; events: unknown[] };
	createdAt: Date;
	completedAt?: Date;
};
...
The `expireAfterSeconds: 0` index on `sessions.expiresAt` lets MongoDB auto-evict expired sessions. The text index on `notes.{title,body}`
powers the search action in the next page.
...
We'll wire up auth in the next page; the dynamic ctx carries just `userId` for now. Sessions live in MongoDB.
...
The `staticContext` opens the MongoDB connection once at boot. The `createContext` looks up the session per request — cheap because of the
`token` unique index.

## Tailwind

Wire Tailwind into the build. The scaffolder produced a `style.css` that imports basic resets; replace it with a Tailwind entry:
...
`vyn build` and `vyn dev` pick up the PostCSS config automatically; Tailwind classes used in route templates and component renders are
...
Open `http://localhost:8000`. You'll see the SPA shell with no route content yet — that's expected; we'll add routes as features land. The
dev server should boot without errors, which proves env parses, MongoDB connects, and the transformer wires up.

If the MongoDB connection fails, check `MONGO_URL` and that the `mongo` container (or whatever you're running) actually listens on 27017.
...
- SuperJSON configured as the wire transformer (so Date / Map / Set / BigInt round-trip cleanly in the coming actions)
- Tailwind ready to style the UI

Continue to **[2 · Notes](../2-notes/)** to build the basic note feature: model, CRUD actions, search, realtime.

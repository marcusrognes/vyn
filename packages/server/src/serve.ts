...
import { loadManifest, makeTryBundle } from "./bundle.ts";
import { identityTransformer, type Transformer } from "./transformer.ts";
import { type BaseCtx, type CookieOpts, EventBus } from "./ctx.ts";
...
export type ServeOpts<S extends object = {}, D extends object = {}> = {
	...
	createContext?: (
		opts: { req: Request; staticCtx: S; baseCtx: BaseCtx },
	) => Promise<D> | D;
...
export async function serve<S extends object = {}, D extends object = {}>(
	opts: ServeOpts<S, D>,
): Promise<ServeHandle<S>> {
	...
	// and cache.
	const indexHtmlBoot = await readFile(join(publicDir, "index.html"), "utf-8")
		.catch(() => undefined);
	...
	if (opts.transport) {
		...
		installPublishHook((name, value) => {
			void Promise.resolve(
				transport.publish(name, { from: instanceId, payload: value }),
			)
		...
		const unsub = await transport.subscribe((name, raw) => {
			...
			const action = registry.get(name) as
				| (Action & { deliverLocal?: (v: unknown) => void })
				| undefined;
			if (
				action?.kind === "subscription" &&
				typeof action.deliverLocal === "function"
			) {
				try {
					action.deliverLocal(env.payload);
				} catch (e) {
					console.warn(`[vyn] transport delivery to ${name} threw:`, e);
				}
			}
		});
	...
	const buildBaseCtx = (request: Request) => {
		...
		const baseCtx: BaseCtx = {
			...
			setHeader(name, value) {
				sideHeaders[name] = value;
			},
			setCookie(name, value, o?: CookieOpts) {
				sideCookies.push(serializeCookie(name, value, o));
			},
		};
	...
	const dispatch = async (
		...
		if (url2.pathname.startsWith("/rpc/")) {
			return handleRpc(request, baseCtx, async (r, b) => {
				const dyn = opts.createContext ? await opts.createContext({ req: r, staticCtx, baseCtx: b }) : ({} as D);
				return { ...staticCtx, ...dyn };
			}, transformer);
		}
		if (url2.pathname === "/mcp" && opts.mcp) {
			return handleMcp(request, async () => {
				const dyn = opts.createContext ? await opts.createContext({ req: request, staticCtx, baseCtx }) : ({} as D);
	...
	if (opts.noListen) {
		console.log(
			`[vyn] boot-only mode (noListen) — actions registered, no HTTP bind`,
		);
		return {
			close: async () => {
				shutdownNotify();
	...
	const denoServer = Deno.serve({
		...
		const { baseCtx, sideHeaders, sideCookies, getStatus } = buildBaseCtx(
			request,
		);
		try {
			let response = await dispatch(request, baseCtx);
			if (!response) response = new Response("not found", { status: 404 });
			return applySideChannel(response, {
				sideHeaders,
				sideCookies,
				sideStatus: getStatus(),
			});
		} catch (e) {
	...
	const counts = registry.list().reduce((acc, a) => {
		acc[a.kind] = (acc[a.kind] ?? 0) + 1;
		return acc;
	}, {} as Record<string, number>);
	const url = `http://${opts.host ?? "localhost"}:${opts.port}`;
	console.log(
		`[vyn] listening on ${url} — ${registry.list().length} actions registered (${
...
function applySideChannel(
	res: Response,
	side: {
		sideHeaders: Record<string, string>;
		sideCookies: string[];
		sideStatus?: number;
	},
): Response {
	if (
		!side.sideStatus && Object.keys(side.sideHeaders).length === 0 &&
		side.sideCookies.length === 0
	) return res;
	...
	return new Response(res.body, {
		status: side.sideStatus ?? res.status,
		headers,
	});
}

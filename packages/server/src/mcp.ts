...
import { type Action, registry } from "@vynjs/core";
...
function promptSpec(action: Action) {
	const args = action.input?.schema as {
...
export async function handleMcp(
	...
	try {
		...
		return jsonRpc(null, { code: -32700, message: "Parse error" });
	}
	...
	try {
		if (body.method === "tools/list") {
			const tools = registry.list().filter((a) => a.tool && !a.tool.hidden).map(
				toolSpec,
			);
		...
		if (body.method === "tools/call") {
			...
			if (!name) {
				return jsonRpc(id, { code: -32602, message: "missing tool name" });
			}
			const action = registry.get(name);
			if (!action || !action.tool) {
				return jsonRpc(id, { code: -32601, message: `unknown tool: ${name}` });
			}
			if (action.kind === "subscription") {
				return jsonRpc(id, {
					code: -32601,
					message: "subscriptions not callable via tools/call",
				});
			}
		...
		if (body.method === "prompts/get") {
			...
			if (!name) {
				return jsonRpc(id, { code: -32602, message: "missing prompt name" });
			}
			const action = registry.get(name);
			if (
				!action ||
				(action.tool as { prompt?: boolean } | undefined)?.prompt !== true
			) {
				return jsonRpc(id, {
					code: -32601,
...
function jsonRpc(
	...
	const payload: McpResponse = error ? { jsonrpc: "2.0", id: id ?? undefined, error } : { jsonrpc: "2.0", id: id ?? undefined, result };

...
function init() {
	document.querySelectorAll<HTMLElement>("[data-anchor]:not([data-anc-wired])")
		.forEach(wire);
}
...
function wire(el: HTMLElement) {
	...
	function reposition() {
		...
		switch (placement) {
			...
			case "bottom-end":
				top = a.bottom + offset;
				left = a.right - e.width;
				break;
			case "top":
				top = a.top - e.height - offset;
				left = a.left + (a.width - e.width) / 2;
				break;
			case "top-start":
				top = a.top - e.height - offset;

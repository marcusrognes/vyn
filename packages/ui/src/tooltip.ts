...
function init() {
	document.querySelectorAll<HTMLElement>("[data-tooltip-for]:not([data-tt-wired])").forEach(wire);
}
...
function wire(tip: HTMLElement) {
	...
	target.setAttribute("aria-describedby", tip.id || (tip.id = `tip_${targetId}`));
	...
	target.addEventListener("mouseenter", () => { timer = setTimeout(show, delay); });

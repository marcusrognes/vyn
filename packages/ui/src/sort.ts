...
function init() {
	document.querySelectorAll<HTMLElement>("[data-sort]:not([data-srt-wired])").forEach(wire);
}
...
function wire(container: HTMLElement) {
	...
	function paint() {
		for (const h of container.querySelectorAll<HTMLElement>("[data-sort-key]")) {
	...
	container.addEventListener("click", (e) => {
		const header = (e.target as HTMLElement).closest<HTMLElement>("[data-sort-key]");
		...
		paint();
		container.dispatchEvent(new CustomEvent("sort", {
			detail: {
				key,
				direction:  next,
				sortState:  [...state.entries()].map(([k, d]) => ({ key: k, direction: d })),
			},
		}));
	});
}

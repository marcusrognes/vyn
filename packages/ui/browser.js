...
export function live(message, opts = {}) {
	...
	if (opts.assertive) { assertive.textContent = message; return; }
...
function registerInit(fn) {
	...
	new MutationObserver(fn).observe(document.body ?? document.documentElement, { childList: true, subtree: true });
}

// ─── keyboard-nav ────────────────────────────────────────────────────
registerInit(() => {
	document.querySelectorAll("[data-keyboard-nav]:not([data-kn-wired])").forEach((container) => {
		...
		container.addEventListener("keydown", (e) => {
			...
			if (k === "Home")  { e.preventDefault(); focusItem(items()[0]); }
			if (k === "End")   { e.preventDefault(); const l = items(); focusItem(l[l.length - 1]); }
			if (k === "Enter" || k === " ") {
...
registerInit(() => {
	document.querySelectorAll("[data-select]:not([data-sel-wired])").forEach((container) => {
		...
		const update = (values) => {
			...
			for (const item of items()) item.setAttribute("aria-selected", values.includes(item.dataset.value) ? "true" : "false");
		...
		container.addEventListener("click", (e) => {
			...
			else { const set = new Set(cur); set.has(v) ? set.delete(v) : set.add(v); update([...set]); }
		});
...
// ─── dismiss ─────────────────────────────────────────────────────────
registerInit(() => {
	document.querySelectorAll("[data-dismiss]:not([data-dis-wired])").forEach((el) => {
		el.dataset.disWired = "true";
		const triggers = (el.dataset.dismissOn ?? "escape outside").split(/\s+/);
		const fire = (reason) => {
			const ev = new CustomEvent("dismiss", { detail: { reason }, cancelable: true });
			el.dispatchEvent(ev);
			if (!ev.defaultPrevented) el.dispatchEvent(new CustomEvent("dismissed", { detail: { reason } }));
		};
		if (triggers.includes("escape")) {
			document.addEventListener("keydown", (e) => { if (e.key === "Escape" && el.contains(document.activeElement)) { e.preventDefault(); fire("escape"); } });
		}
		if (triggers.includes("outside")) {
			document.addEventListener("pointerdown", (e) => { if (!el.contains(e.target)) fire("outside"); });
		}
		if (triggers.includes("focus-out")) {
			el.addEventListener("focusout", () => setTimeout(() => { if (!el.contains(document.activeElement)) fire("focus-out"); }, 0));
		}
	});
});
...
// ─── typeahead ───────────────────────────────────────────────────────
registerInit(() => {
	document.querySelectorAll("[data-typeahead]:not([data-ta-wired])").forEach((container) => {
...
// ─── copy ────────────────────────────────────────────────────────────
registerInit(() => {
	document.querySelectorAll("[data-copy]:not([data-copy-wired])").forEach((el) => {
		...
		el.addEventListener("click", async (e) => {
			...
			try { await navigator.clipboard.writeText(text); el.dataset.state = "copied"; live("Copied"); setTimeout(() => delete el.dataset.state, 1500); }
			catch { live("Copy failed", { assertive: true }); }
		});
	});
});
...
// ─── auto-resize ─────────────────────────────────────────────────────
registerInit(() => {
	document.querySelectorAll("textarea[data-auto-resize]:not([data-ar-wired])").forEach((ta) => {
		...
		const resize = () => {
			...
			const h = Math.min(Math.max(ta.scrollHeight, lh * minRows), lh * maxRows);
...
// ─── sort ────────────────────────────────────────────────────────────
registerInit(() => {
	document.querySelectorAll("[data-sort]:not([data-srt-wired])").forEach((container) => {
		...
		const paint = () => {
			for (const h of container.querySelectorAll("[data-sort-key]")) {
				...
				h.setAttribute("aria-sort", dir === "asc" ? "ascending" : dir === "desc" ? "descending" : "none");
			}
		};
		paint();
		container.addEventListener("click", (e) => {
			...
			paint();
			container.dispatchEvent(new CustomEvent("sort", { detail: { key, direction: next, sortState: [...state.entries()].map(([k, d]) => ({ key: k, direction: d })) } }));
		});
	});
});
...
if (!customElements.get("v-toaster")) {
	class VToasterElement extends HTMLElement {
		...
		connectedCallback() {
			...
			const [v, h] = pos.split("-");
			if (v === "top") this.style.top = "0"; else this.style.bottom = "0";
			if (h === "start" || h === "left") this.style.left = "0";
			else if (h === "end" || h === "right") this.style.right = "0";
			else { this.style.left = "0"; this.style.right = "0"; this.style.alignItems = "center"; }
		}
		show(toast) {
			const id = ++this.#seq;
			const el = document.createElement("div");
			const palette = { info: "#1e293b", success: "#15803d", warning: "#b45309", error: "#b91c1c" };
			const bg = palette[toast.kind ?? "info"];
			el.setAttribute("role", toast.kind === "error" || toast.kind === "warning" ? "alert" : "status");
			el.style.cssText = `background:${bg};color:white;padding:0.75rem 1rem;border-radius:0.5rem;max-width:24rem;box-shadow:0 4px 10px rgba(0,0,0,0.15);pointer-events:auto;cursor:pointer;font-family:system-ui,sans-serif`;
			...
			live(`${toast.title ? toast.title + ". " : ""}${toast.body}`, { assertive: toast.kind === "error" });

...
function memStore(): InboxStore & { _rows: InboxRow[] } {
	...
	return {
		_rows: rows,
		async insertOne(row) { rows.push(row); return row; },
		async find(filter) {
			return rows.filter((r) => r.userId === filter.userId && (filter.readAt === undefined || (filter.readAt === null ? r.readAt === null : r.readAt !== null)));
		},
		async countDocuments(filter) {
			return rows.filter((r) => r.userId === filter.userId && (filter.readAt === undefined || (filter.readAt === null ? r.readAt === null : r.readAt !== null))).length;
		},
		...
		async updateMany(filter, patch) {
			rows.filter((r) => r.userId === filter.userId && r.readAt === null).forEach((r) => { r.readAt = patch.$set.readAt; });
		},
	};
}
...
describe("@vyn/notify-inbox", () => {
	it("send persists a row with default fields", async () => {
		...
		await adapter.send({ notification: "x.y", payload: { title: "T" } }, { userId: "u1" });
	...
	it("groupedWith is persisted when present in the payload", async () => {
		const store = memStore();
		const adapter = inboxAdapter({ collection: store });
		await adapter.send({ notification: "x", groupedWith: ["a", "b"], payload: {} }, { userId: "u1" });

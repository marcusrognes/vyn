...
describe("notify-runtime", () => {
	beforeEach(() => { _resetNotifyRuntime(); });

	it("instant: dispatches immediately via adapter", async () => {
		const sent: unknown[] = [];
		installNotify({
			adapters: { push: { send: async (p) => { sent.push(p); } } },
		});
		const n = createNotification({
			channels: { push: { mode: "instant", render: async (o: any) => ({ title: "T", body: o.input.body }) } },
		});
	...
	it("deferred: enqueues, delivers after delay", async () => {
		...
		const n = createNotification({
			channels: { push: { mode: "deferred", delay: 50, render: async (o: any) => ({ title: "A" }) } },
		});
	...
	it("bundling: two deferred items in coalesce window become one delivery", async () => {
		const sent: unknown[] = [];
		installNotify({
			adapters: { email: { send: async (p) => { sent.push(p); } } },
			coalesceWindowMs: 200,
		});
		const n1 = createNotification({
			name: "n1",
			channels: { email: { mode: "deferred", delay: 30, render: async () => ({ html: "<a>" }) } },
		});
		const n2 = createNotification({
			name: "n2",
			channels: { email: { mode: "deferred", delay: 50, render: async () => ({ html: "<b>" }) } },
		});
	...
	it("preferences resolver: disables a channel for a user", async () => {
		...
		installNotify({
			...
			preferences: async (userId) =>
				userId === "u-quiet" ? { push: { enabled: false } } : { push: { enabled: true } },
		});
		const n = createNotification({
			name: "pings",
			channels: { push: { mode: "instant", render: async () => ({ title: "ping" }) } },
		});
	...
	it("preferences override flips digest mode to instant", async () => {
		...
		const n = createNotification({
			name: "digestNote",
			channels: { email: { mode: "digest", digestKey: (i: any) => i.userId, defaultCron: "0 8 * * *", renderItem: async () => ({}), renderDigest: async () => ({ subject: "x" }), render: async () => ({ subject: "instant" }) } },
		});
		await n.send({ userId: "u1" });
		await new Promise((r) => setTimeout(r, 10));
		expect(sent.length).toBe(1);
	...
	it("queue size grows on enqueue, shrinks on flush", async () => {
		...
		const n = createNotification({
			channels: { email: { mode: "deferred", delay: 60_000, render: async () => ({}) } },
		});
		await n.send({ userId: "u1" });

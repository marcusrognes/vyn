...
function localParts(instant: Date, tz: string): { y: number; mo: number; d: number; h: number; mi: number; dow: number } {
...
function matches(cron: CronExpression, lp: ReturnType<typeof localParts>): boolean {
...
export function previousTick(cron: CronExpression, from: Date, tz: string, windowMs = 366 * 24 * 60 * 60_000): Date | undefined {
...
export function nextTick(cron: CronExpression, from: Date, tz: string, windowMs = 366 * 24 * 60 * 60_000): Date | undefined {
	const start = from.getTime();
	const limit = start + windowMs;

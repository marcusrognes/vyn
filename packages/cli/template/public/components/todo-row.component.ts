...
import { component, type Html, html, render } from "@vynjs/client";
...
export default component<Props>((el) => {
	el.render = () => {
		...
		render(
			...
			html`
				<label>
					<input type="checkbox" ${el.props.done
						? html`
							checked
						`
						: ""}>
	...
	el.addEventListener("change", () => {
		void rpc.todos.toggle.mutate({ id: el.props.id });
	});

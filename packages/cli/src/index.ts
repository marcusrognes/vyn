...
`],
		["features/hello/hello.actions.ts", `import { createQuery, v } from "@vynjs/core";
...
`],
		["public/main.ts", `import { createApp, $ } from "@vynjs/client";
...
async function gen() {
	...
	const nsAliases: { ns: string; alias: string }[] = [];
	let seq = 0;
	for (const [ns, files] of byNs) {
		...
		nsAliases.push({ ns, alias: aliases.join(" & typeof ") ? `(typeof ${aliases.join(" & typeof ")})` : "{}" });
	}
	...
	for (const { ns, alias } of nsAliases) {
		lines.push(`\t${JSON.stringify(ns)}: ${alias};`);
	}
	lines.push("};");
	lines.push("");

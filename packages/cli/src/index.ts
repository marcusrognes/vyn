...
//
// Runtime detection:
//   - If `deno.json` (or `deno.jsonc`) is present in cwd, dev/check
//     spawn `deno` instead of `node`; init scaffolds a Deno project.
//   - Override with `--runtime=node|deno` on any subcommand, or set
//     VYN_RUNTIME in the environment.
...
const argv = process.argv.slice(2).filter((a) => !a.startsWith("--runtime="));
const explicitRuntime =
	process.argv.slice(2).find((a) => a.startsWith("--runtime="))?.split("=")[1] ??
	process.env.VYN_RUNTIME ??
	"";
const cmd = argv[0];

type Runtime = "node" | "deno";
function detectRuntime(): Runtime {
	if (explicitRuntime === "node" || explicitRuntime === "deno") return explicitRuntime;
...
Usage:
  vyn init [--runtime=deno]   scaffold a hello-world app in the current dir
  vyn dev                     run the dev server (watch + reload)
  vyn build                   build the production bundle
  vyn check                   type-check the project (tsc or 'deno check')
...
Runtime: ${RUNTIME} (auto-detected; set VYN_RUNTIME or --runtime to override)
`);
}
...
`;
	const styleCss = `body { font-family: system-ui, sans-serif; padding: 2rem; }`;
	const mainJs   = `console.log("hello vyn");`;
	const genStub  = `// generated by 'vyn gen' — do not edit.\nexport {};\n`;

	const files: Array<[string, string]> = RUNTIME === "deno"
		? [
			["deno.json", JSON.stringify({
				...
				imports: {
					"@vyn/core":   "npm:@vyn/core",
					"@vyn/server": "npm:@vyn/server",
					"@vyn/client": "npm:@vyn/client",
				},
				nodeModulesDir: "auto",
...
`],
		...
		: [
			["package.json", JSON.stringify({
				name,
				type: "module",
				scripts: { dev: "vyn dev", build: "vyn build", check: "vyn check", gen: "vyn gen" },
...
async function dev() {
	...
	const [bin, args] = RUNTIME === "deno"
		? ["deno", ["run", "-A", "--watch", "--unstable-node-globals", "--unstable-bare-node-builtins", "server.ts"]]
		: ["node", ["--experimental-strip-types", "--watch", "server.ts"]];
	const proc = spawn(bin, args as string[], {
		stdio: "inherit",
...
async function check() {
	const [bin, args] = RUNTIME === "deno"
		...
		: ["npx", ["-y", "tsc", "--noEmit"]];
	const proc = spawn(bin, args as string[], { stdio: "inherit" });
